...
Trains a linear classification head (frozen backbone) on cardiac MRI using the
same patient-level pooling as the logistic regression probe: mean-pool ED
slices + mean-pool ES slices → concatenate → linear head.
...
def finetune_sweep_and_train(
...
) -> tuple[float, nn.Module, ClassificationHead, list[dict], StandardScaler]:
    """Sweep LR via stratified k-fold CV (or val split), then retrain on all training data.

    The LR sweep and final training are performed with a **frozen backbone**
    using pre-extracted features.  This is orders of magnitude faster than
    running the backbone forward per epoch and is standard practice in
    ...
    Args:
        ...
        pooling: ``"cls"`` for CLS token, ``"gap"`` for global average pooling.
    ...
    Returns:
        ...
        backbone: Backbone (unchanged, frozen).

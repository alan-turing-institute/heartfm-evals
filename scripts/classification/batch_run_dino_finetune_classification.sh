#!/usr/bin/env bash
#SBATCH --job-name=heartfm-cls-dino-finetune
#SBATCH --array=0-17
...
# DINOv3 finetune classification array job.
...
echo "=== Task ${SLURM_ARRAY_TASK_ID}: dataset=${DATASET} backbone=dinov3 model=${MODEL} eval=finetune pooling=${POOLING} ==="

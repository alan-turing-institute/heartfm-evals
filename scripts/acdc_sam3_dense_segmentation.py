"""
Dense Probe: Pixel-Level Cardiac Segmentation with SAM 3 on ACDC

Architecture: Frozen SAM 3 vision encoder -> cached image embeddings ->
...
print(f"SAM 3 source: hub -> {SAM3_MODEL_ID}")
...
def extract_sam3_features(sam3_model, image_processor, image_2d, device):
    # image_2d: (H, W) in [0, 1]
    ...
    feats = sam3_model.get_image_embeddings(pixel_values)
...
def cache_sam3_features(
    ...
    for sample_idx in tqdm(range(len(cinema_dataset)), desc="Caching SAM 3 features"):
        ...
        for z in range(n_slices):
            ...
            feats = extract_sam3_features(sam3_model, image_processor, image_2d, device)
            ...
            if augment and n_augments > 0:
                for aug_idx in range(n_augments):
                    ...
                    aug_feats = extract_sam3_features(
                        sam3_model, image_processor, aug_img, device
                    )
                    torch.save(
...
train_manifest = cache_sam3_features(
    sam3_model,
    image_processor,
    train_cinema,
    CACHE_DIR / "train",
...
val_manifest = cache_sam3_features(
    sam3_model, image_processor, val_cinema, CACHE_DIR / "val", DEVICE
)

print("\nCaching test features...")
test_manifest = cache_sam3_features(
    sam3_model, image_processor, test_cinema, CACHE_DIR / "test", DEVICE
)
...
probe = SAMDenseProbe(in_channels=in_channels).to(DEVICE)

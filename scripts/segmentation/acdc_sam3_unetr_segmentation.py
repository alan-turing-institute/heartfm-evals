"""
Dense UNetR Probe: Pixel-Level Cardiac Segmentation with SAM 3 on ACDC

Architecture: Frozen SAM 3 ViT encoder -> cached per-layer intermediate features
-> stacked along z to form pseudo-3D volumes -> 3D UNetR decoder -> class logits
(BG, RV, MYO, LV).

Note: SAM 3 is a gated model on HuggingFace. Request access at:
...
# -- Backbone config: SAM 3 uses a ViT-H encoder (32 blocks, embed_dim=1280) --
EMBED_DIM = 1280
...
# -- Cache --
CACHE_DIR = Path("../../feature_cache/sam3_unetr3d")
...
print(f"Backbone: {SAM3_MODEL_ID} (embed_dim={EMBED_DIM})")
...
def extract_sam3_volume_features(
    sam3_model,
    processor,
    sax_volume,
    layer_indices,
    ...
    """Extract per-layer SAM 3 ViT features for a SAX volume, stacked along z."""
    ...
    for z in range(target_depth):
        ...
        enc_out = sam3_model.vision_encoder(pixel_values, output_hidden_states=True)
        hidden_states = (
            enc_out.hidden_states
        )  # tuple of (B, 64, 64, embed_dim) — channels-last
        ...
        for idx in layer_indices:
            ...
            per_layer[idx].append(feat.squeeze(0).cpu())  # (embed_dim, gs, gs)
...
def cache_sam3_volume_features(
    sam3_model,
    processor,
    cinema_dataset,
    cache_dir,
    ...
    """Cache SAM 3 volume features for all patients/frames."""
    out_dir = Path(cache_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    manifest = []

    for sample_idx in tqdm(
        range(len(cinema_dataset)), desc="Caching SAM 3 volume features"
    ):
        ...
        features_dict, padded_image, actual_slices = extract_sam3_volume_features(
            sam3_model,
            processor,
            image_3d,
            layer_indices,
...
# -- Load SAM 3 and Cache Volume Features --
image_processor = Sam3Processor.from_pretrained(
    SAM3_MODEL_ID,
...
print(f"Loaded SAM 3 from: {SAM3_MODEL_ID}")
print(f"Frozen parameters: {sum(p.numel() for p in sam3_model.parameters()):,}")

print("Caching training volume features...")
train_manifest = cache_sam3_volume_features(
    sam3_model,
    image_processor,
    train_cinema,
    CACHE_DIR / "train",
...
val_manifest = cache_sam3_volume_features(
    sam3_model,
    image_processor,
    val_cinema,
    CACHE_DIR / "val",
...
test_manifest = cache_sam3_volume_features(
    sam3_model,
    image_processor,
    test_cinema,
    CACHE_DIR / "test",
...
plt.tight_layout()
plt.savefig("sam3_unetr3d_training_curves.png", dpi=150)
plt.close()
...
plt.tight_layout()
plt.savefig("sam3_unetr3d_test_predictions.png", dpi=150)
plt.close()


# -- Save Model --
save_path = Path("dense_unetr3d_probe_sam3.pt")
torch.save(
    {

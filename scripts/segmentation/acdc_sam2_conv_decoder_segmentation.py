"""
Dense Probe: Pixel-Level Cardiac Segmentation with SAM 2 on ACDC
...
parser = argparse.ArgumentParser(
    description="Dense segmentation probe with SAM 2 on ACDC"
)
parser.add_argument(
    "--model",
...
plt.savefig(f"{CACHE_NAME}_training_curves_{timestamp}.png", dpi=150)
plt.close()
...
plt.tight_layout()
plt.savefig(f"{CACHE_NAME}_test_predictions_{timestamp}.png", dpi=150)
plt.close()


# -- Save Model --
save_path = Path(f"dense_probe_{CACHE_NAME}_{timestamp}.pt")
torch.save(
    {

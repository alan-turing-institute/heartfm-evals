#!/usr/bin/env bash
#SBATCH --job-name=heartfm-seg-dino-conv-decoder
#SBATCH --array=0-8
...
# DINOv3 conv_decoder segmentation array job.
...
echo "=== Task ${SLURM_ARRAY_TASK_ID}: dataset=${DATASET} backbone=dinov3 model=${MODEL} decoder=conv_decoder ==="

#!/usr/bin/env bash
#SBATCH --job-name=heartfm-seg-sam-linear-probe
#SBATCH --array=0-8
...
# SAM v1 linear_probe segmentation array job.
...
echo "=== Task ${SLURM_ARRAY_TASK_ID}: dataset=${DATASET} backbone=sam model=${MODEL} decoder=linear_probe ==="

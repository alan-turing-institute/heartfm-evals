"""
Dense Probe: Pixel-Level Cardiac Segmentation with SAM on ACDC
...
LR = 1e-3
WEIGHT_DECAY = 1e-4
TRAIN_AUG_COPIES = 1  # number of augmented copies per training slice
N_EPOCHS = 20
PATIENCE = 10
...
if SAM_LOCAL_DIR is not None:
    print(f"SAM source: local -> {SAM_LOCAL_DIR}")
else:
    print(f"SAM source: hub -> {SAM_MODEL_ID} (auto_download={AUTO_DOWNLOAD})")
...
def extract_sam_features(sam_model, image_processor, image_2d, device):
    # image_2d: (H, W) in [0, 1]
    ...
    # (1, C=256, h=64, w=64) for sam-vit-base
    feats = sam_model.get_image_embeddings(pixel_values)
...
def cache_sam_features(
    ...
    for sample_idx in tqdm(range(len(cinema_dataset)), desc="Caching SAM features"):
        ...
        for z in range(n_slices):
            ...
            feats = extract_sam_features(sam_model, image_processor, image_2d, device)
...
train_manifest = cache_sam_features(
    sam_model,
    image_processor,
    train_cinema,
    CACHE_DIR / "train",

...
**Note**: Classification scripts are NOT modified in this plan (colleague working on separate branch). The shared helpers below are sourced from segmentation code; classification scripts will be rewired to import from here in a separate follow-up.
...
- **Shared preprocessing helpers** (currently duplicated across classification + segmentation):
  ...
  - `preprocess_sam_slice(image_2d, image_processor)` — grayscale→PIL→SAM processor (extract from SAM2 segmentation scripts; same logic exists in `classification_probe.cache_sam_cls_features`)
  - `pad_volume_z(vol, target_depth)` — z-padding/truncation (move from `dense_unetr_probe._pad_volume_z`; same logic exists in `classification_probe._extract_cinema_volume_token`)
- **Backbone-type dispatch helper**: `get_backbone_type(config) -> str` returning `"dinov3"` / `"cinema"` / `"sam2"` (inspired by the `backbone_type` dispatch pattern in `finetune_classification.extract_patient_feature()`)
...
- **Metadata helpers** (re-implement here for use by segmentation; classification has duplicates that will be rewired later):
  - `get_pathology_map(meta_df) -> dict[str, str]` — used by segmentation for val-split stratification (same logic exists in `classification_probe.py`)
  - `_group_samples_by_patient(cinema_dataset, pathology_map) -> list[dict]` — useful for patient-level grouping (same logic exists in `finetune_classification.py`)
...
- `run_training_loop(model, train_loader, val_loader, config) -> (best_model_state, history)` — full loop with early stopping, scheduling, checkpointing. Generalise the epoch→train→eval→track-best→early-stop pattern (similar to `finetune_classification._train_with_lr()`, used as design reference only — that file is not modified).
...
- **NOT modified** (colleague's branch): `src/heartfm_evals/classification_probe.py` and `finetune_classification.py` — these keep their own duplicated backbone/preprocessing logic for now. They will be rewired to import from the new shared modules in a follow-up after the colleague's classification branch merges.
...
- **Deferred (classification follow-up)**: `classification_probe.py` and `finetune_classification.py` are NOT touched — a colleague is working on classification on a separate branch. Once that branch merges and the new shared modules exist, a follow-up PR will rewire classification to import `preprocess_slice`, `pad_volume_z`, `preprocess_sam_slice`, `get_pathology_map`, `_group_samples_by_patient` from the new modules. The shared modules are designed with both tasks in mind so this rewiring is mechanical.
...
2. **Gradient checkpointing**: CineMA's pipeline uses `model.set_grad_ckpt(True)` for memory efficiency during fine-tuning. Should we add this? Recommend: yes, as a simple boolean in `training/finetune.yaml`.

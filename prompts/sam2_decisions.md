...
**segmentation**. This document records the decisions made when extending it to
**classification**, plus the analysis behind them, and the analysis of a proposed
segmentation multi-scale change that was ultimately **not** adopted.

---

## Status on `main`

> **Sections 1-6 describe what is implemented.** SAM2 classification (Stage 4 GAP,
> `cls_embed_dim`) is live in `run_classification.py`.
>
> **Section 7 was considered and deliberately NOT applied**, and the "SAM2 (new)" rows of
> section 8 describe that unadopted proposal. `SAM2_CONFIGS.layer_indices` on `main` remains
> the original all-Stage-3 selection - `(4, 6, 8, 10)` / `(4, 7, 11, 14)` /
> `(6, 11, 16, 21)` / `(9, 21, 33, 44)` - and there is no `stage_embed_dims` field.
>
> **Why:** adopting it changes SAM2 segmentation features, which invalidates the committed
> `results/segmentation/*/sam2_*` numbers and the summaries built from them, and it requires
> non-uniform-channel support in `DINOv3UNetRDecoder` and `DenseLinearProbe`. The limitation
> section 7 identifies is real - all four "multi-scale" skips sit at one resolution - so this
> is worth revisiting as its own change, with its own re-extraction and regression pass, not
...
| File | Change |
|------|--------|
| `src/heartfm_evals/backbones.py` | Added `cls_embed_dim` to `SAM2_CONFIGS`; exposed it from `_load_sam2`. Added `SAM_CONFIGS` (SAM v1 per-depth `layer_indices`) and exposed `layer_indices` from `_load_sam` |
| `src/heartfm_evals/classification_probe.py` | Added `cache_sam2_cls_features` (Stage 4 GAP) |
| `src/heartfm_evals/features.py` | Added `extract_sam_2d_features` (SAM v1 per-slice, for segmentation) |
| `src/heartfm_evals/caching.py` | Added `cache_sam_2d_features` (SAM v1 per-slice cache) |
| `scripts/classification/run_classification.py` | Added `sam2` backbone choice, `--sam2-model-id`, routing to the new cache fn, `cls_embed_dim` reporting |
| `scripts/segmentation/run_segmentation.py` | Added `sam` backbone choice, `--sam-model-id`, routing to `cache_sam_2d_features` / `cache_sam_volume_features` |

Net effect: **all four backbones (`dinov3`, `cinema`, `sam`, `sam2`) now work for both
...
SAM v1 was previously classification-only. Two gaps blocked segmentation:

- **No per-slice extractor.** `extract_sam_volume_features` (3D) already worked for SAM v1 -
  it is in fact written for it - but the 2D decoders had no equivalent.
  `extract_sam_2d_features` mirrors `extract_sam2_2d_features`: run the vision encoder with
  `output_hidden_states=True`, permute channels-last to channels-first, bilinearly downsample
  each layer to `grid_size=12`, concatenate along channels.
- **No `layer_indices`.** `_load_sam` returned no `layer_indices`, so `run_segmentation.py`
  silently fell back to its `(3, 6, 9, 11)` default - fine for ViT-B's 12 blocks but sampling
  only the first third of ViT-L (24) and ViT-H (32). `SAM_CONFIGS` now records evenly spaced
  quartiles per depth:
...
Unlike Hiera, the SAM v1 ViT is isotropic - every block emits the same channel count at
64x64 - so no per-stage channel bookkeeping is needed and `DINOv3UNetRDecoder` works
unmodified. `embed_dim` is still read from `vision_config.hidden_size` so the loaded weights
stay the source of truth.

Also note `extract_sam_volume_features` serves **both** SAM families: both vision encoders
emit channels-last hidden states that are interpolated to `grid_size`, so only the native
resolutions differ and those are normalised away.
...
SAM2_CONFIGS = {
    "facebook/sam2.1-hiera-tiny": {
        "embed_dim":     384,             # Stage 3 - used by segmentation
        "cls_embed_dim": 768,             # Stage 4 - used by classification
        "layer_indices": (4, 6, 8, 10),   # all Stage 3 (see "Status on main")
    },
    ...
}
```
...
|-------|---------|-------------|
| `embed_dim` | Segmentation (all four `layer_indices` are Stage 3) | Stage 3 channel count |
| `cls_embed_dim` | Classification (`cache_sam2_cls_features`) | Stage 4 channel count |

There is no `stage_embed_dims` field on `main` - it belongs to the unadopted section 7
proposal.
...
> Not yet added on `main` - the SAM2 classification batch scripts described here were
> removed along with the rest of the SAM2 support and have not been restored.
...
## 7. Segmentation `layer_indices` fix - truly multi-scale features (NOT APPLIED)

> **Proposal only - not implemented on `main`.** See "Status on `main`" above for why.
> The Hiera stage analysis below is accurate and is the reason to revisit this later.
...
> The **"SAM2 (new)"** rows below describe the unadopted section 7 proposal. On `main`,
> SAM2 segmentation behaves as the **"SAM2 (old)"** rows describe.
